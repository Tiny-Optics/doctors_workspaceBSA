<template>
  <div class="p-6">
    <div class="mb-8 flex justify-between items-center">
      <div>
        <h1 class="text-3xl font-bold text-gray-900">Working Party Management</h1>
        <p class="text-gray-600 mt-2">Manage Working Party categories</p>
      </div>
      ...
      <div class="flex items-start">
        ...
        <div class="ml-3 flex-1">
          ...
          <p class="text-sm text-yellow-700 mt-1">
            Dropbox must be configured before creating or managing Working Party categories.
            Categories are stored in the WORKING_PARTIES/ folder in Dropbox.
    ...
    <div v-else-if="!loading && categories.length === 0" class="bg-white rounded-xl shadow-lg p-12 text-center">
      ...
      <h3 class="text-xl font-semibold text-gray-900 mb-2">No Working Party Categories Yet</h3>
      <p v-if="dropboxConfigured" class="text-gray-600 mb-6">Get started by creating your first Working Party category.</p>
      <p v-else class="text-gray-600 mb-6">Configure Dropbox first, then create your Working Party categories.</p>
          ...
          <form @submit.prevent="submitForm" class="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g., Anemia Working Party"
              />
            </div>
...
import type { WorkingPartyCategory, WorkingPartyFile, CreateCategoryRequest, UpdateCategoryRequest } from '@/types/workingParty'
import { useToast } from '@/composables/useToast'
import { dropboxAdminService } from '@/services/dropboxAdminService'

const toast = useToast()

const categories = ref<WorkingPartyCategory[]>([])
...
const editingCategory = ref<WorkingPartyCategory | null>(null)
const categoryToDelete = ref<WorkingPartyCategory | null>(null)
const selectedCategory = ref<WorkingPartyCategory | null>(null)
...
async function loadCategories() {
  loading.value = true
  try {
    const response = await workingPartyService.listCategories({ limit: 100 })
...
function openEditModal(category: WorkingPartyCategory) {
  if (!dropboxConfigured.value) {
...
async function submitForm() {
  ...
  try {
    if (imageFile.value) {
      const imagePath = await workingPartyService.uploadImage(imageFile.value)
    ...
    if (isEditMode.value && editingCategory.value) {
      ...
      await workingPartyService.updateCategory(editingCategory.value.id, updateData)
      toast.success(`Successfully updated "${formData.value.name}" category!`)
    } else {
      await workingPartyService.createCategory(formData.value)
...
async function toggleActive(category: WorkingPartyCategory) {
  const action = category.isActive ? 'deactivated' : 'activated'
  try {
    await workingPartyService.updateCategory(category.id, { isActive: !category.isActive })
    await loadCategories()
...
function confirmDelete(category: WorkingPartyCategory) {
...
async function deleteCategory() {
  ...
  try {
    await workingPartyService.deleteCategory(categoryToDelete.value.id)
    await loadCategories()
...
async function viewFiles(category: WorkingPartyCategory) {
  selectedCategory.value = category
  showFilesModal.value = true
  loadingFiles.value = true
  try {
    const fileData = await workingPartyService.getCategoryFiles(category.id)
...
async function downloadFile(file: WorkingPartyFile) {
  ...
  try {
    const downloadLink = await workingPartyService.getFileDownloadLink(selectedCategory.value.id, file.name)
